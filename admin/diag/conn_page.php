...
$oses = array(0 => "mswindows", 1 =>"linux", 2 =>"macos", 3 =>"kali", 4 =>"ubuntu", 5 =>"debian", 6 => "other");
            ...
            <div class="col-md-10 box-widget widget-user-2">
                <div class="widget-user-header">
                    ...
                    <p class="widget-user-desc">
                        <?php echo $requestHandler->decryptData($conn['username']) . '@' . $conn['hostname'] .':'. $conn['port']; ?>
                    </p>
        ...
        <div class="tab-content" style="margin-left: -10px !important;">
            <div class="tab-pane fade show active" id="acc_info" role="tabpanel">
                <div class="box box-body member-form">
                    <form id="connInfoForm" name="connInfoForm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                        <input type="hidden" name="data" value="<?php echo $requestHandler->protect(array('action', 'update', $conn['id']));?>">
                        <div id="vm-create-menuform" class="row pt-4 mx-auto" style="max-width: 80%"> 
                            <div class="col-md-6 form-floating mb-3">
                                <input class="form-control" id="name" name="name" value="<?php echo $conn['name']; ?>">
                                <label class="control-label required text-dark ps-3">Name</label>
                            </div>
                            <div class="col-md-6 form-floating mb-3">
                                <input class="form-control" id="host" name="host" value="<?php echo $conn['hostname']; ?>">
                                <label class="control-label required text-dark ps-3">Host</label>
                            </div>
                            <div class="col-md-6 form-floating mb-3">
                                <input class="form-control" id="port" name="port" value="<?php echo $conn['port']; ?>">
                            ...
                            <div class="col-md-6 form-floating mb-3">
                                <input class="form-control" id="username" name="username" value="<?php echo $requestHandler->decryptData($conn['username']); ?>">
                                <label class="control-label required text-dark ps-3">Username</label>
                            </div>
                            <div class="col-md-6 form-floating mb-3">
                                <input type="password" class="form-control" id="password" name="password" value="<?php echo $requestHandler->decryptData($conn['password']); ?>">
                            ...
                            <div class="col-md-6 form-floating mb-3">
                                <select class="form-select" id="os" name="os" aria-label="operating system of the VM">
                                    ...
                                    <?php foreach ($oses as $os) { ?>
                                        <option <?php echo ($os == $conn['os']) ? 'selected' : ''; ?> value="<?php echo $os; ?>"><?php echo $os; ?></option>
                                    <?php } ?>
                                </select>
                            ...
                            <div class="col-md-6 form-floating mb-3">
                                <select class="form-select" id="owner" name="owner" aria-label="the owner of the new connection">
                                    ...
                                    <?php foreach ($userHandler->getAllUsers() as $user) { ?>
                                        <option <?php echo ($user['id'] == $conn['owner']) ? 'selected' : ''; ?> value="<?php echo $user['id']; ?>"><?php echo $user['username']; ?></option>
                                    <?php } ?>
                                </select>
                                ...
                                <div class="form-floating">
                                    <select class="form-select" id="node" name="node" aria-label="the node that contains the VM">
                                        ...
                                        <?php foreach ($proxmox->getNodes()->data as $node) { ?>
                                            <option <?php echo ($node->node === $conn['node']) ? 'selected' : ''; ?> value="<?php echo $node->node; ?>"><?php echo $node->node; ?></option>
                                        <?php } ?>
                                    </select>
                            ...
                            <div class="col-md-6 form-floating mb-3">
                                <input class="form-control" id="drive" name="drive" placeholder="" value="<?php echo $conn['drive']; ?>">
                                <label for="drive" class="text-dark ps-3">Shared Drive</label>
                            </div>
                        </div>     
                    </form>
                    <div align="center" class="box-footer">
                        <button type="submit" form="connInfoForm" class="btn btn-primary member-form-submit">Save</button>
                    </div>
                </div>
            </div>
        </div>
